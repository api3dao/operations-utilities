...
export const serverETHValue = 723.39202;
export const serverBTCValue = 41091.12345;

const PROTOCOL_ID_PSP = "2";
const subscriptionIdETH =
  "0xc1ed31de05a9aa74410c24bccd6aa40235006f9063f1c65d47401e97ad04560e";
const subscriptionIdBTC =
...
const roles = {
  deployer: new hre.ethers.Wallet(localConfigETH.privateKeys.deployer).connect(
    provider
  ),
  manager: new hre.ethers.Wallet(localConfigETH.privateKeys.manager).connect(
    provider
  ),
  sponsor: new hre.ethers.Wallet(localConfigETH.privateKeys.sponsor).connect(
    provider
  ),
  randomPerson: new hre.ethers.Wallet(
    localConfigETH.privateKeys.randomPerson
  ).connect(provider),
};
...
export const updateBeacon = async (
  ...
  const timestampAndSignature = await getTimestampAndSignature(
    airnodeWallet,
    subscriptionId,
  ...
  await dapiServer
    ...
    .fulfillPspBeaconUpdate(
      ...
      hre.ethers.utils.defaultAbiCoder.encode(
        ["int256"],
        [hre.ethers.BigNumber.from(apiValue)]
      ),
...
export const deployAndUpdateSubscriptions = async (
  startingValueETH: number,
  startingValueBTC: number
) => {
  ...
  const templateIdETH = hre.ethers.utils.solidityKeccak256(
    ["bytes32", "bytes"],
    [endpointIdETH, parametersETH]
  );

  // Subscriptions
  const thresholdETH = (await dapiServer.HUNDRED_PERCENT()).div(
    localConfigETH.threshold
  ); // Update threshold %
  const beaconUpdateSubscriptionConditionParametersETH =
    hre.ethers.utils.defaultAbiCoder.encode(["uint256"], [thresholdETH]);
  const beaconUpdateSubscriptionConditionsETH = [
    ...
    {
      type: "bytes",
      name: "_conditionParameters",
      value: beaconUpdateSubscriptionConditionParametersETH,
    },
  ];
  const encodedBeaconUpdateSubscriptionConditionsETH = abi.encode(
    beaconUpdateSubscriptionConditionsETH
  );
  await dapiServer
  ...
  const templateIdBTC = hre.ethers.utils.solidityKeccak256(
    ["bytes32", "bytes"],
    [endpointIdBTC, parametersBTC]
  );

  // Subscriptions
  const thresholdBTC = (await dapiServer.HUNDRED_PERCENT()).div(
    localConfigBTC.threshold
  ); // Update threshold %
  const beaconUpdateSubscriptionConditionParameters2 =
    hre.ethers.utils.defaultAbiCoder.encode(["uint256"], [thresholdBTC]);
  const beaconUpdateSubscriptionConditionsBTC = [
    ...
    },
    {
      type: "bytes",
      name: "_conditionParameters",
      value: beaconUpdateSubscriptionConditionParameters2,
    },
  ];
  const encodedBeaconUpdateSubscriptionConditionsBTC = abi.encode(
    beaconUpdateSubscriptionConditionsBTC
  );
  await dapiServer
    .connect(roles.randomPerson)
    .registerBeaconUpdateSubscription(
  ...
  const apiValueETH = Math.floor(startingValueETH * 1_000_000);
  const apiValueBTC = Math.floor(startingValueBTC * 1_000_000);
  // ETH subscription
  await updateBeacon(
    dapiServer,
    airnodePspSponsorWallet,
  ...
  const signedDataValue =
    "0x000000000000000000000000000000000000000000000000000000002bff42b7";
  const { timestamp: signedDataTimestamp, signature: signedDataSignature } =
    await signData(airnodeWallet, templateIdETH, signedDataValue);
  const signedData = {
    data: {
