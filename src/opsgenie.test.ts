...
import { evaluateThreshold } from './opsgenie';
import { Metric, OpsGenieConfig, OutputMetric } from './types';
import { buildTelemetryConfig } from '../test/fixtures/config';

const opsGenieConfig = buildTelemetryConfig().opsGenieConfig as OpsGenieConfig;
...
describe('alert utilities', () => {
  describe('evaluateThreshold', () => {
    it("tests for an alert being generated if a beacon's deviation is over the configured threshold", async () => {
      ...
      const message = await evaluateThreshold(metric, fnToCall, opsGenieConfig);
